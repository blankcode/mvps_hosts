...
cd /etc/MVP; # go to tmp dir.
  # Sadly I don't see a way to cownload just a checksum so, we have to grab the file each time.
...
[[ "$NEW_FILE" == "$OLD_HOSTS" ]] || { 
  cat HOSTS > /etc/hosts; rm HOSTS hosts.zip; # replace the /etc/hosts file with the new one if the new one is different.
